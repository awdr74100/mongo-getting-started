...
$ db.contacts.createIndex({ "dob.age": 1 }) # 創建單字段索引 (索引鍵的升降序不重要，MongoDB 可從任一方向遍歷索引)
$ db.contacts.createIndex({ "dob.age": 1, gender: 1 }) # 創建複合索引 (存在排序順序之分)(索引可由左至右組合使用，必須與第一個鍵搭配)(查詢鍵順序不影響排列順序)
```
