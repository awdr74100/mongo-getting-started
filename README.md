...
- 添加索引可使查詢、更新、刪除在符合索引條件時盡可能快地運行 (每次更新或刪除也都需要先做一個查詢步驟)。
- 集合中擁有越多索引可使更多查詢受益，但相對寫入的性能也將逐漸降低，在每次執行寫入操作時將一併更新所有索引，這需要花費額外的時間來完成。
- 索引如同數據也將被複製到 WiredTiger 內部快取中，更多的索引也意味著內部快取的壓力更大，這種壓力會導致更多的驅逐發生。
- 僅創建必要的索引，而不是一味地創建可能需要的索引，甚至更不該出現重複索引 (可從它索引前綴解決的索引) 及未使用索引 (經長時間運行但依然未使用索引)，衡量索引對於查詢及寫入的影響將可更有效的控制性能。
