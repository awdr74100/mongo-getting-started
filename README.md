...
```shell
$ db.users.updateOne({ name: "Chris" }, { $set: { name: "sharon" }}) # 根據頂級字段更新其值 (不存在即新增)(支持數組、嵌入文檔、文檔數組)
$ db.users.updateMany({ "hobbies.title": "Sports" }, { $unset: { isSporty: "" } }) # 根據頂級字段刪除其值 (任意輸入值)(陣列項目變為 null)(支持數組、嵌入文檔、文檔數組)
```
